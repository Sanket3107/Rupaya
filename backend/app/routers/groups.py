...
async def create_new_group(
    group_data: GroupCreate, current_user: UserOut = Depends(get_current_user)
):
...
async def get_group(group_id: str, current_user: UserOut = Depends(get_current_user)):
    return await get_group_detail(group_id, current_user.id)


@router.post("/{group_id}/members", response_model=GroupMemberOut)
async def add_member(
    group_id: str, member_data: AddMemberRequest, current_user: UserOut = Depends(get_current_user)
):
...
async def remove_member(
    group_id: str, member_id: str, current_user: UserOut = Depends(get_current_user)
):
