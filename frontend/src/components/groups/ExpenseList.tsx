...
export function ExpenseList({ groupId, currentUserId, onAddExpense, refreshTrigger = 0 }: ExpenseListProps) {
    ...
    const fetchInitialBills = async () => {
        setLoading(true);
        try {
            const response = await api.get<PaginatedResponse<Bill>>(`/bills/group/${groupId}?skip=0&limit=${ITEMS_PER_PAGE}`);
    ...
    const fetchMoreBills = async () => {
        ...
        setLoadingMore(true);
        try {
            const response = await api.get<PaginatedResponse<Bill>>(`/bills/group/${groupId}?skip=${skip}&limit=${ITEMS_PER_PAGE}`);
